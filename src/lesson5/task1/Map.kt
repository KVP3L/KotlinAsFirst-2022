...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    ...
    for ((name, tel) in map) {
        val k = mapA [name]
        if (k != null && tel != k) newMap[name] = "$k, $tel"
        else newMap[name] = tel
    }
    return newMap
}
...
val numberAndIndex = mutableMapOf<Int, Int>()
    for (index in list.indices) {
        ...
        if (delta in numberAndIndex.keys) {
            return numberAndIndex[delta]!! to index
        } else numberAndIndex [list[index]] = index
    }
    return -1 to -1
}
